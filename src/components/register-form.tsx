...
type Props = {
    label: string,
    onCall: (data: RegisterFormData) => void
}

export default function RegisterForm({ label, onCall }: Props) {
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.policy}>
                ...
                <Pressable onPress={() => router.push("/policy/tos")}>
                    <Text style={{ color: "#2563eb" }}>Terms of Service</Text>
                </Pressable>

                <Text> and </Text>

                <Pressable onPress={() => router.push("/policy/pp")}>
                    <Text style={{ color: "#2563eb" }}>Privacy Policy</Text>
                </Pressable>
            </View>


            <PrimaryButton label={label} onCall={handleSubmit(onCall)} size={22} icon="chevron-forward-outline"/>
        </View>
    );
}
